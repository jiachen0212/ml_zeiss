...
def report_best_scores(results, ind, n_top=3):
    ...
    with open(r'./parameter_{}_emb.json'.format(ind), 'w') as js_file:
        js_file.write(data)
...
def cross_val(X_train, Y_train, x_test, y_test, ind, cx_lab_curve, important_inds):
    ...
    parameters = json.load(open(r'./parameter_{}_emb.json'.format(ind), 'r'))
...
def generate_data(ds, important_inds, x_data_file, y_data_file, sensor_thick_2_OvenNo, OvenNoPair_Labs, Ovens_embedding_sensor_feature):
...
def overfiting(X, Y):
    ...
    param1 = json.load(open(r'./parameter_{}_emb.json', 'r'))
    num_round = 200
...
def eval_test_data(x_test, important_inds, y_tests, title='train data'):
    xgb_models = []
    for i in range(11):
        params = json.load(open(r'./parameter_{}_emb.json'.format(i), 'r'))
...
def Test(x_test, slim_best_cx_curve, important_inds, sensor_thick_2_OvenNo, OvenNoPair_Labs, save_dir, embedding_sensor_feature_dim):
    aa = [important_ind * 5 + 380 for important_ind in important_inds]
    xgb_models = []
    for i in range(11):
        params = json.load(open(r'./parameter_{}_emb.json'.format(i), 'r'))
    ...
    for iindex, single_test_data in enumerate(x_test):
        ...
        # search thickness
        finetune_thre1 = [0.1, 0.1, 0.8, 1.5, 1.5]
...
if __name__ == '__main__':
    ...
    x_data_file = r'./X_embedding.npy'
    y_data_file = r'./Y_embedding.npy'
    sensor_thick_2_OvenNo = r'D:\work\project\卡尔蔡司AR镀膜\ML_ZEISS-dev\data\cx_data_result\sensor_emb_thick_2_oveNno.json'
    OvenNoPair_Labs = r'D:\work\project\卡尔蔡司AR镀膜\ML_ZEISS-dev\data\cx_data_result\OvenNoPair_emb_Labs.json'

    # load embedding sensor_feature
    sensor_embedding_feature = np.load(r'./168dims_sensor_feature.npy').tolist()
    # sensor_embedding_feature = np.load(r'./8dims_sensor_feature.npy').tolist()
    Ovens = np.load(r'./ACT_V1_MEISSNER_POLYCOLDTEMP_ovens.npy').tolist()
    embedding_sensor_feature_dim = len(sensor_embedding_feature[0])
    ...
    assert len(Ovens_embedding_sensor_feature) == len(Ovens)
    ...
    for ind, important_ind in enumerate(important_inds):
        ...
        # random_state 固定, 保证每个频段的训测数据拆分是一致的
        ...
        if ind == 0:
            np.save(r'./x_test_emb.npy', X_test)
            np.save(r'./x_train_emb.npy', X_train)
    ...
    # eval test_data
    # x_test = np.load(r'./x_train.npy')
    # eval_test_data(x_test, important_inds, y_trains, title='train data')
    x_test = np.load(r'./x_test_emb.npy')
    scaled_thickness = np.load(r'./scaled_thickness.npy')
    # eval_test_data(x_test, important_inds, y_tests, title='test data')

    save_dir = r'D:\work\project\卡尔蔡司AR镀膜\ML_ZEISS-dev\data\embedding_model_and_real'
    #1. model_pred and real_cuvre check
    compare_pred_realCurve(x_test, scaled_thickness, save_dir)

    #2. embedding thickness search
    # save_dir = r'D:\work\project\卡尔蔡司AR镀膜\ML_ZEISS-dev\data\1103_li_gong_review_change_seed'
    ...
    # Test(x_test, slim_best_cx_curve, important_inds, sensor_thick_2_OvenNo, OvenNoPair_Labs, save_dir, embedding_sensor_feature_dim)
