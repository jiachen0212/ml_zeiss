...
def run(train_x, train_y, model, train_dataloader, epochs, best, is_train=True, optimizer=None):
    if is_train:
        loss_list = []
        ...
        #         show_y_pred(y_pred, gt_y=train_y, epo=epoch + 1, best=best)
...
if __name__ == "__main__":
    ...
    optimizer_train = optimizers.Adam(model.parameters(),
                                      ...
                                      betas=(0.9, 0.999), amsgrad=True)
    if flag == 1:
        run(train_x, train_y, model, train_dataloader, epochs_train, best, optimizer=optimizer_train)
    elif flag == 0:
        run(train_x, train_y, model, train_dataloader, epochs_finetune, best, is_train=False)
        compare_res(best)
    else:
