...
def compare_res(best):
    ...
    plt.plot(x, y1, color='cornflowerblue', label='R4 curve')
    plt.plot(x, y2, color='lightpink', label='modified curve')
    plt.plot(x, best, color='red', label='R1')
    plt.legend()
...
def test_model1_and_model2(mean_thick, std_thick, mean_sensor, std_sensor, flag=None):
    '''
...
# 反向验证
def modify_std(mean_sensor, std_sensor, best):
    # std -> lab
    final_js = r'D:\work\project\卡尔蔡司AR镀膜\0731data\7.31\7.31\finally.json'
    f = json.load(open(final_js, 'r'))
    data = f['33388888811']
...
if __name__ == "__main__":
    ...
    # test_model1_and_model2(mean_thick, std_thick, mean_sensor, std_sensor, flag=1)
    # test_model1_and_model2(mean_thick, std_thick, mean_sensor, std_sensor, flag=2)

    # # 反向验证: 一条不ok的样本, 先过model2, 再过model1, 看看膜厚调整值的变化
    # modify_std(mean_sensor, std_sensor, best)
    # thicks = modify_thickness(mean_thick, std_thick)
    # print(thicks)
