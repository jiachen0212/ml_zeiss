...
def report_best_scores(results, ind, seed, pca, n_top=3):
    ...
    with open(r'./parameter_last_{}_seed{}_pca{}.json'.format(ind, seed, pca), 'w') as js_file:
        js_file.write(data)
...
def cross_val(model_flag, X, y, X_test, test_y, ind, important_inds, dim_mse, dim_mse_):
    ...
    if model_flag == 0:
        parameters = json.load(open(r'./parameter_last_{}_seed{}_pca{}.json'.format(ind, seed, pca), 'r'))
...
def train(seed, pca, model_flag, x_train, y_train, x_test, y_test, important_inds, all_data,
    ...
    X_test, Y_test, test_oven = tmp_generate_test_data(pca, seed, [all_data[-1]], important_inds, x_test, y_test,
                                                       test_feature)
...
def compare_pred_realCurve(seed, pca, pre_cur_oven, x_test, scaled_thickness, lstm_tsf_ender_thick_2_Oven, Oven_lab,
                           save_dir):
    all_weighted_metric = []
    ...
    xgb_models = []
    for i in range(ll):
        params = json.load(open(r'./parameter_last_{}_seed{}_pca{}.json'.format(i, seed, pca), 'r'))
...
def forward_model(ff, seed, pca, test_oven, save_dir, all_data, test_feature_dict):
    ...
    X_test_data, lstm_tsf_ender_thick_2_Oven, Oven_real_lab, scaled_thickness, pre_cur_oven, cur_oven_thick = generate_test_data(
        [all_data[-1]], test_oven,
        important_inds,
        test_feature_dict)
    oven_pred_lab, eval_metric = compare_pred_realCurve(seed, pca, pre_cur_oven, X_test_data, scaled_thickness,
                                                        lstm_tsf_ender_thick_2_Oven,
    ...
    for iindex, single_test_data in enumerate(x_test):
        ...
        def get_deta_function_and_best(thickness_list):
            ...
            for ml_model in ml_models:
                ...
                model_pred_deta.append(ml_model.predict(np.array([test]))[0])
            ...
            cur_function = function_thick2lab(scaled_thickness)
            function_minus_best_curve = [cur_function[important_ind] - cx_lab_curve[important_ind] for important_ind in
                                         important_inds]
            slim_function = [cur_function[ind] for ind in important_inds]

            return function_minus_best_curve, model_pred_deta, slim_function
        ...
        def evaluate_config(config):
            ...
            function_minus_best_curve, model_pred_deta, slim_function = get_deta_function_and_best(
                [config['x1'], config['x2'], config['x3'], config['x4'], config['x5']])
            weights = [0] * ll
            weights[2], weights[9], weights[4], weights[12], weights[13] = 1, 1, 2, 2, 2
            model_pred_m = [slim_function[ind] - model_pred_deta[ind] for ind in range(ll)]
            # plt.plot([i for i in range(14)], model_pred_m, label='model_pred')
            # plt.legend()
            # plt.show()
            metric = weighted_metric(function_minus_best_curve, model_pred_deta, weights, model_pred_m)
        ...
        plt.plot(aa, pred_curve_slim, color='dimgray', linewidth=1.0,
                 label='pred_curve, thickness diff: {}'.format(
...
def curve_trend_check(pca, save_dir, x_test, lstm_tsf_ender_thick_2_Oven, scaled_thickness):
    ...
    for iindex, single_test_data in enumerate(x_test):
        ...
        plt.plot(aa, pred_m_modified, label='layer5-1, model_predicate', color='pink')
        plt.legend()
...
if __name__ == '__main__':
    ...
    all_data = [pd.read_csv(data1), pd.read_csv(data2), pd.read_csv(data3), pd.read_csv(data4), pd.read_csv(data5),
                pd.read_csv(data6)]
    ...
    for pca in pcas:
        for seed in seeds:
            ...
            if flag == "train and forward":
                # 1.train model
                ml_models, test_oven = run_train(seed, pca, train_feature, test_feature, model_flag, important_inds,
                                                 all_data)
                X_test_data, lstm_tsf_ender_thick_2_Oven, scaled_thickness, oven_pred_lab, Oven_real_lab, pre_cur_oven = \
