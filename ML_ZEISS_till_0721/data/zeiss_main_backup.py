...
train_sensor_feature = json.load(open(r'./no_thick_kpca_train_seed{}pca{}.json'.format(seed, pca), 'r'))
test_sensor_feature = json.load(open(r'./no_thick_kpca_test_seed{}pca{}.json'.format(seed, pca), 'r'))
...
import numpy as np
from pass_utils import xgb_hyperparameter_searching, cross_val, load_model1_model2, eval_metric
...
def generate_data(all_train_data, train_sensor_feature):
    '''
...
def train_model1(X, model1_y, ff_train):
    ...
    for i in range(ll):
        ...
        # xgb_hyperparameter_searching(X, y, i, seed, pca, 'model1')
        model1 = cross_val(X, y, i, seed, pca, ff_train, 'model1')
...
def train_model2(model1s, model2_x, model2_y, ff_train):
    ...
    for i in range(ll):
        ...
        # xgb_hyperparameter_searching(X, y, i, seed, pca, 'model2')
        model2 = cross_val(X, y, i, seed, pca, ff_train, 'model2')
...
def forward_model_check(save_dir, lab_mse_txt, model1_x, model1_y, model2_x, model2_y, data_flag=None):
    ...
    # train data 上抽一些数据做曲线拟合测试, 模型应该是见过ta们的
    if data_flag == "train":
        print('train data ..')
...
def run():
    ...
    global flag, important_inds, seed, best_cx_curve, slim_best_cx_curve, pca, bad_ovens
    flag = sys.argv[1:][0]
    bad_ovens = ["33121090910", "33121110302", "33321091006", "33321090806", "33121090508", "33121090603",
                 "33121090601"]
    ...
    seeds = [33]
    pcas = [10]  # [10, 20, 50, 100, 200, 300, 500]
    ...
    all_train_data = [pd.read_csv(data1), pd.read_csv(data2), pd.read_csv(data3), pd.read_csv(data4), pd.read_csv(data5)]
    online_test_data = pd.read_csv(data6)

    train_mse_txt = r'./train.txt'
    test_lab_mse = r'./test_lab_mse_test.txt'
    os.remove(train_mse_txt)
    os.remove(test_lab_mse)
    if not os.path.exists(train_mse_txt):
        ff_train = open(train_mse_txt, 'w')
    if not os.path.exists(test_lab_mse):
        test_lab_mse_txt = open(test_lab_mse, 'w')
    for seed in seeds:
        for pca in pcas:
            train_sensor_feature = json.load(open(r'./no_thick_kpca_train_seed{}pca{}_pass.json'.format(seed, pca), 'r'))
            test_sensor_feature = json.load(open(r'./no_thick_kpca_test_seed{}pca{}_pass.json'.format(seed, pca), 'r'))
            ...
            if flag == '1':
                ...
                try:
                    ...
                    print("loss model1 train data")
                    continue
                model1s = train_model1(model1_x, model1_y, ff_train)
                print("train model1 done.~  start train model2 ~")
                try:
...
if __name__ == '__main__':
    ...
    run()
    # flag: 0, generate data
    # flag: 1, train model1, model2
    # flag: 2, model forward test
    # flag: 3, model_trend test
    # flag: 4, online search thickness
