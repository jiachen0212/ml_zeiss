...
class DataLoader():
    ...
    def __init__(self, base_dir, evt_dir, process_file, match_file, data_detail_file, math_file_title,
        ...
        # self.titles = ["OvenNo", "FileID", "CCCX", "Thickness", "Material", "Start_time", "Step_start_timestamp", "Consumables", "single_lab_curve", "single_lab_value", "path_sensor_data", "clean_cycle_index",
        #                "Type"]
        self.titles = ["BatchNo", "FileName", "CCCX", "Thickness", "Material", "Start_time", "Consumables", "single_lab_curve", "single_lab_value", "path_sensor_data", "clean_cycle_index",
                       "Type"]

        # {炉号:[evt_cx, evtcc]}
        self.BatchNo_EvtPair = dict()
        self.Oven_Type = dict()
        self.BatchNo_CX_LabCurve = dict()
        self.BatchNo_CC_LabCurve = dict()
        self.all_OvenNo_CCCX_LabCurve = dict()
        self.double_LabCurve = dict()
        self.FileName_LabCurve = dict()
    ...
    def get_thickness(self, evt_file):
        ...
        '''
        csv_file = os.path.join(self.base_dir, self.evt_dir, 'EVT{}.csv'.format(evt_file))
    ...
    def get_start_time(self, evt_file):
        ...
        with open(csv_file, 'r') as f:
            lines = f.readlines()
            if self.check_evts(lines, csv_file):
                for line in lines:
                    if 'Production Start' in line:
        ...
        for i in range(1, rows):
            ...
            if len(curve) > 0:
                ...
                if CX_or_CC.lower() == "cx":
                    self.BatchNo_CX_LabCurve[OvenNo] = [curve, [L, A, B]]
                elif CX_or_CC.lower() == "cc":
                    self.BatchNo_CC_LabCurve[OvenNo] = [curve, [L, A, B]]
    ...
    def get_lab_curve(self, ovenno_cx, ovenno_cc):
        ...
        for ovenno in ovenno_cx:
            ovenno = ovenno.split('_')[0]
            ovenno = int(ovenno)
            try:
                curve_value = self.BatchNo_CX_LabCurve[str(ovenno)]
        ...
        for ovenno in ovenno_cc:
            ...
            ovenno = int(ovenno)
            try:
                curve_value = self.BatchNo_CC_LabCurve[str(ovenno)]
    ...
    def get_consumables_info(self, OvenNo, CX_or_CC):
        ...
        for i, ovenno in enumerate(OvenNo):
            key = '{}_{}'.format(ovenno, CX_or_CC)
            try:
                consumable = consumable_dict[key]
    ...
    def get_sensor_data(self, evt_csv):
        ...
        '''
        file_path = os.path.join(self.base_dir, self.evt_dir, '{}.csv'.format(evt_csv))
        # try:
    ...
    def get_data_type(self, oven_cxcc):
        oven = oven_cxcc.split('_')[0]
        try:
            type_ = self.Oven_Type[oven]
        except:
            type_ = ''
        return type_
    ...
    def split_data_pair(self, pd_data):
        ...
        lens = len(pd_data)
        for i in range(lens - 1):
            evtname1, evtname2 = int(pd_data["FileID"][i][3:]), int(pd_data["FileID"][i + 1][3:])
            evt1, evt2 = evtname1 % 1000, evtname2 % 1000
    ...
    def mix_cx_cc(self, df_match):
        ...
        ovencxcc = []
        for i in range(lens - 1):
            evtname1, evtname2 = df_match[self.file_id][i], df_match[self.file_id][i + 1]
            evt1, evt2 = evtname1%1000, evtname2 % 1000
            if abs(evt1 - evt2) == 1:
                oven1, oven2 = df_match[self.ovenno][i], df_match[self.ovenno][i + 1]
                cx_cc1, cx_cc2 = oven1.split('_')[-1], oven2.split('_')[-1]
                if (cx_cc1 + cx_cc2).lower() in ["cccx", "cxcc"]:
                    # 连续且正背面跳转:
                    if evtname1 not in cx_cc_evts:
                        cx_cc_evts.append(evtname1)
                        ovencxcc.append(oven1)
                    if evtname2 not in cx_cc_evts:
                        cx_cc_evts.append(evtname2)
                        ovencxcc.append(oven2)
        slim_evts = df_match.FileName.apply(lambda x: x in cx_cc_evts).reset_index(drop=True)
        slim_df = df_match[slim_evts].reset_index(drop=True)
        # 返回所有的evt, 然后以evt为index获取lab_value, lab_curve, clean_index等信息f
        # lens = len(slim_df)
        # for i in range(lens):
        #     oven, evt, face = slim_df[self.ovenno][i], slim_df[self.file_id][i], slim_df[self.cx_or_cc][i].split("_")[-1]
            # if oven not in self.BatchNo_EvtPair:
            #     self.BatchNo_EvtPair[oven] = ['', '']
            # if face.lower() == "cx":
            #     self.BatchNo_EvtPair[oven][0] = evt
            # elif face.lower() == "cc":
            #     self.BatchNo_EvtPair[oven][1] = evt
    ...
    def get_double_lab_curve(self, oven):
        try:
            curve = self.double_LabCurve[oven][0]
        except:
            curve = ''
    ...
    def get_double_lab_value(self, oven):
        try:
            value = self.double_LabCurve[oven][1]
        except:
            value = ''
    ...
    def evt_continue(self, pd_data):
        ...
        lens = len(pd_data)
        for i in range(lens - 1):
            evtname1, evtname2 = pd_data["FileName"][i], pd_data["FileName"][i + 1]
            # 取evt_name的后三位
            evt1, evt2 = evtname1 % 1000, evtname2 % 1000
            if abs(evt2 - evt1) <= 2:
                if evt1 not in evts:
    ...
    def mix_face_post_process(self, data_path):
        ...
        evts, faces, types = data[self.file_id], data["CCCX"], data["Type"]
        lens = len(data)
        for i in range(lens - 1):
        ...
        slim_evt = data.FileName.apply(lambda x: x in slim_evts).reset_index(drop=True)
    ...
    def run_load_data(self, ):
        '''
            ...
            math_file_title = {"炉号": "BatchNo", "创建时间": ["StartDate", "StartTime"], "正背面": "Recipe_name"}
            single_lab_curve_file_title = {"膜色曲线": "DataExpand", "OvenNo_CXCC": "测膜标签", "LAB_value": ["L值", "A值", "B值"]}

        '''

        # 一些文件的 title_key
        self.ovenno = self.math_file_title["炉号"]
        self.cx_or_cc = self.math_file_title["正背面"]
        self.file_id = self.math_file_title["evt_name"]
        self.start_day_time = self.math_file_title["创建时间"]
        self.OvenNo_cx_or_cc_title = self.single_lab_curve_file_title['OvenNo_CXCC']
        self.lab_value_title = self.single_lab_curve_file_title['LAB_value']
        self.single_curve_title = self.single_lab_curve_file_title['膜色曲线']

        # 获取炉号的双面曲线
        self.get_double_curve(self.OvenNo_cx_or_cc_title, self.lab_value_title)
        # 分别获取正背面lab曲线
        self.get_single_curve(self.OvenNo_cx_or_cc_title, self.lab_value_title, self.single_curve_title)
        ...
        # 33炉号筛选
        slim_ovens = df_match.BatchNo.apply(lambda x: str(x)[:2] == str(self.glasses_type)).reset_index(drop=True)
        df_match = df_match[slim_ovens].reset_index(drop=True)
        # evtname 去重
        df_match = df_match.drop_duplicates(subset=[self.file_id], keep='first').reset_index(drop=True)
        # 筛选具有evt和sensor, 且同时是thickness_layer层镀膜的文件
        file_list = self.evt_and_sensor_thickness()
        selected_oven = df_match.FileName.apply(lambda x: str(x) in file_list).reset_index(drop=True)
        ...
        continue_ovens = df_match.FileName.apply(lambda x: x in continue_evts).reset_index(drop=True)
        all_continue_df = df_match[continue_ovens].reset_index(drop=True)
        ovens_face= all_continue_df.BatchNo 
        all_ovencxcc_list = ["{}".format(ovens_face[i]) for i in range(len(all_continue_df))]
        all_continue_df['clean_cycle_index'] = self.get_cycle_value(all_ovencxcc_list, all_OvenNo_cycle)

        # 添加type信息
        all_continue_df['Type'] = all_continue_df.BatchNo.apply(self.get_data_type)
        path_ = os.path.join(self.save_dir, '{}_all.xlsx'.format(self.csv_name))

        # mac
        # writer = pd.ExcelWriter(path_, encoding="utf-8-sig")
        # winds
        writer = pd.ExcelWriter(path_)
        all_continue_df.to_excel(writer, "sheet1")
        writer.save()

        # 场景1, 连续且正背跳转
        ovencxcc_list, slim_df = self.mix_cx_cc(df_match)   
        ovens, faces = [a.split("_")[0] for a in ovencxcc_list], [a.split("_")[1] for a in ovencxcc_list]
        self.df_cxcc[self.ovenno] = slim_df.BatchNo
        self.df_cxcc[self.file_id] = slim_df.FileName
        self.df_cxcc["CCCX"] = faces
        self.df_cxcc["start_time"] = ["{}:{}".format(slim_df.StartDate[i], slim_df.StartTime[i]) for i in range(len(slim_df))]
        ...
        self.df_cxcc['double_lab_curve'] = self.df_cxcc.BatchNo.apply(self.get_double_lab_curve)
        self.df_cxcc['double_lab_value'] = self.df_cxcc.BatchNo.apply(self.get_double_lab_value)
        self.df_cxcc['Thickness'] = self.df_cxcc.FileName.apply(self.get_thickness)
        self.df_cxcc['Material'] = self.df_cxcc.FileName.apply(self.get_material)
        self.df_cxcc['Start_time'] = self.df_cxcc.FileName.apply(self.get_start_time)
        self.df_cxcc['Step_start_timestamp'] = self.df_cxcc.FileName.apply(self.get_step_timestamp)
        self.df_cxcc['Consumables'] = self.get_consumables_info(ovens, faces)
        self.df_cxcc['path_sensor_data'] = self.df_cxcc.FileName.apply(self.get_sensor_data)
        self.df_cxcc['clean_cycle_index'] = self.get_cycle_value(ovencxcc_list, all_OvenNo_cycle)
        # 添加老花近视信息
        self.df_cxcc['Type'] = self.df_cxcc.BatchNo.apply(self.get_data_type)

        # 场景2, 拆分正背面数据
        CXCC = df_match[self.cx_or_cc]
        ...
        curve_cx, LAB_cx, selected_oven_cx, curve_cc, LAB_cc, selected_oven_cc = self.get_lab_curve(all_CX.BatchNo,
                                                                                             all_CC.BatchNo)
        selected_cx = all_CX.BatchNo.apply(lambda x: int(x.split('_')[0]) in selected_oven_cx).reset_index(drop=True)
        df_match_cx = all_CX[selected_cx].reset_index(drop=True)
        selected_cc = all_CC.BatchNo.apply(lambda x: int(x.split('_')[0]) in selected_oven_cc).reset_index(drop=True)
        df_match_cc = all_CC[selected_cc].reset_index(drop=True)
        Oven_CX, Oven_CC = ["{}_{}".format(a.split('_')[0], "CX") for a in df_match_cx.BatchNo], ["{}_{}".format(a.split('_')[0], "CC") for a in
                                                                                  df_match_cc.BatchNo]

        # 分别添加炉号, EvtName FileID, Create_Time, Recipe_name CX-CC 信息 [按照0519之前的方式命名.]
        # cx
        oven_cxs = [a.split('_')[0] for a in df_match_cx.BatchNo]
        self.df_cx["OvenNo"] = oven_cxs
        self.df_cx["FileID"] = ["EVT{}".format(a) for a in df_match_cx[self.file_id]]
        self.df_cx['Create_Time'] = df_match_cx.UniteTime
        self.df_cx["CCCX"] = 'CX'
        self.df_cx['Thickness'] = df_match_cx.FileName.apply(self.get_thickness)
        self.df_cx['Material'] = df_match_cx.FileName.apply(self.get_material)
        self.df_cx['Start_time'] = df_match_cx.FileName.apply(self.get_start_time)
        self.df_cx['Step_start_timestamp'] = df_match_cx.FileName.apply(self.get_step_timestamp)
        self.df_cx['Consumables'] = self.get_consumables_info(oven_cxs, "CX")
        ...
        self.df_cx['path_sensor_data'] = df_match_cx.FileName.apply(self.get_sensor_data)
        self.df_cx['clean_cycle_index'] = self.get_cycle_value(Oven_CX, OvenNo_cycle_cx)
        self.df_cx['Type'] = df_match_cx.BatchNo.apply(self.get_data_type)

        # cc
        oven_ccs = [a.split('_')[0] for a in df_match_cc.BatchNo]
        self.df_cc["OvenNo"] = oven_ccs
        self.df_cc["FileID"] = ["EVT{}".format(a) for a in df_match_cc[self.file_id]]
        self.df_cc['Create_Time'] = df_match_cc.UniteTime
        self.df_cc["CCCX"] = 'CC'
        self.df_cc['Thickness'] = df_match_cc.FileName.apply(self.get_thickness)    
        self.df_cc['Material'] = df_match_cc.FileName.apply(self.get_material)
        self.df_cc['Start_time'] = df_match_cc.FileName.apply(self.get_start_time)
        self.df_cc['Step_start_timestamp'] = df_match_cc.FileName.apply(self.get_step_timestamp)
        self.df_cc['Consumables'] = self.get_consumables_info(oven_ccs, "CC")
        ...
        self.df_cc['path_sensor_data'] = df_match_cc.FileName.apply(self.get_sensor_data)
        self.df_cc['clean_cycle_index'] = self.get_cycle_value(Oven_CC, OvenNo_cycle_cc)
        self.df_cc['Type'] = df_match_cc.BatchNo.apply(self.get_data_type)
...
def merge_all_csvs(csv_name, code_dir, dirs):

    base_dir = os.path.join(code_dir, 'out')
    csvs = []
    for dir_ in dirs:
        files = os.listdir(os.path.join(base_dir, dir_))
        files = [os.path.join(base_dir, dir_, a) for a in files]
        csvs.extend([a for a in files if (a.split('.')[-1] == 'csv' and 'mix' not in a)])
    frames = []
    for csv in csvs:
    ...
    len1 = len(data)
    titles = ["OvenNo", "FileID", "CCCX", "Thickness", "Material", "Start_time", "Step_start_timestamp", "Consumables", "single_lab_curve", "single_lab_value", "path_sensor_data", "clean_cycle_index", "Type"]
    for title in titles:
        data = data[data[title].notna()]
    len2 = len(data)
    data.to_csv(csv_name, index=False)
    print("before len: {}, slimed len: {}".format(len1, len2))


if __name__ == "__main__":

    code_dir = r'D:\mac_air_backup\chenjia\Download\Smartmore\2021\1110_beijing\ML_ZEISS-cj-dev\data'
    root_dir = r'D:\mac_air_backup\chenjia\Download\Smartmore\2021\1110_beijing\shanghai_data\膜厚推优\ML_ZEISS-dev'
    
    # flag: 0: merge data, 1: generate data 
    flag = 0

    if flag == 0:
        dirs = ['2.14data', '2.25data', '3.07data', '3.18data', '3.28data', '4.11data', '4.19data', '4.25data', '5.10data', '5.18data', '5.23data', '6.1data', '6.6data', '6.15data', '6.21data', '6.30data', '7.7data', '7.14data', '7.21data']
        all_train, all_test = './till_0714_train.csv', './till_0714_test.csv'
        # 留下最新一次日期的数据做test不merge
        merge_all_csvs(all_train, code_dir, dirs[:-1])
        merge_all_csvs(all_test, code_dir, [dirs[-1]])
    
    elif flag == 1:
        dirs = ['7.21data']
        for dir_ in dirs:
            base_dir = os.path.join(root_dir, dir_)
            evt_dir = 'EVT'
            ...
            save_dir = os.path.join(code_dir, 'out', csv_name)
            ...
            math_file_title = {"炉号": "BatchNo", "创建时间": ["StartDate", "StartTime"], "正背面": "Recipe_name", "evt_name": "FileName"}
            single_lab_curve_file_title = {"膜色曲线": "DataExpand", "OvenNo_CXCC": "测膜标签", "LAB_value": ["L值", "A值", "B值"]}
            data_detail_title = {"炉号": "炉序号", "老花近视1.6_1.67": "产品名"}
