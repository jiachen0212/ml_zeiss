...
def evt_thick_and_lab(evts, thicknesses, curevs, faces, types, save_dir):
    ...
    for i in range(lens-1):
        evt1, evt2 = int(evts[i][-3:]), int(evts[i+1][-3:])
...
def check_thick_and_curve(save_dir, compare_res_dir):
    ...
    for csv in csvs:
        ...
        try:
            ...
            continue
        evts, thicknesses, single_curves, faces, types = [a for a in data['FileID']], [a for a in data['Thickness']], [a for a in data[
...
def clean_zeiss_review(save_dir, zeiss_res_dir, xlsx_name):
    ...
    for sub_type_dir in sub_type_dirs:
        ...
        if len(ims) > 0:
            ...
            org_len = len(data) // 2
            slim_evt = data.FileID.apply(lambda x: str(x) not in evts).reset_index(drop=True)
    ...
    # 高亮那些 bad evts
    bad_evts = [a[3:] for a in bad_evts]
