...
def generate_test_data(datas, col_name):
    OvenNos = []
    X = []
    ...
    Ovens = np.array(OvenNos)
    np.save(r"./test_Ovens.npy", Ovens)
    np.save(r"./{}_data5_tf_X.npy".format(col_name), X)
...
def generate_data(col_name, datas, std_mean_txt):
    ...
    if os.path.exists(r'./tf_X_{}.npy'.format(col_name)):
        return np.load(r'./tf_X_{}.npy'.format(col_name)), np.load(r"./Ovens.npy")
    else:
        OvenNos = []
        ...
        OvenNos = np.array(OvenNos)
        np.save(r"./Ovens.npy", OvenNos)
...
def train_tf_lstm_to_get_col_feature_list(sensor_cols, datas, embedd_ed_dim):
    ...
    for col_name in sensor_cols:
        ...
        X, OvenNos = generate_data(col_name, datas, std_mean_txt)
...
def train_data_lstm_feature(data_train, sensor_cols):
    # 1. train model and get tf_features
    embedd_ed_dim = 18
    train_Ovens = train_tf_lstm_to_get_col_feature_list(sensor_cols, data_train, embedd_ed_dim)
    ...
    return all_tf_features, train_Ovens


def test_data_lstm_feature(data_test, sensor_cols):
    all_test_tf_features, Ovens = None, None
    for col_ind, col_name in enumerate(sensor_cols):
        model = load_model('tf_model_{}.h5'.format(col_name))
        X_col, Ovens = generate_test_data(data_test, col_name)
        data_size, sqe_len = X_col.shape[0], X_col.shape[1]
        X_col = X_col.reshape((data_size, sqe_len, -1))

        get_3rd_layer_output = K.function([model.layers[0].input],
                                          [model.layers[0].output])
        layer_output = get_3rd_layer_output([X_col])[0]
        if col_ind == 0:
            all_test_tf_features = layer_output
        else:
            all_test_tf_features = np.concatenate((all_test_tf_features, layer_output), axis=1)

    np.save('./data5_test_tf_lstm_feature.npy', all_test_tf_features)

    return all_test_tf_features, Ovens
...
if __name__ == '__main__':
    ...
    data_train = [pd.read_csv(data1), pd.read_csv(data2), pd.read_csv(data3), pd.read_csv(data4)]
    data_test = [pd.read_csv(data5)]
    ...
    train_lstm_feature, train_Ovens = train_data_lstm_feature(data_train, sensor_cols)

    # test_lstm_feature
    test_lstm_feature, test_Ovens = test_data_lstm_feature(data_test, sensor_cols)