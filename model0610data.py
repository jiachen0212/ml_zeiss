...
def normal_dir_name(dir_):
    dir_1 = r'D:\work\project\卡尔蔡司AR镀膜\卡尔蔡司AR模色推优数据_20210610\33#机台文件1'  # 这个随意写都行..
    if not os.path.exists(dir_1):
        os.mkdir(dir_1)
    for name in os.listdir(dir_):
        print(name, name.lower())
        f1 = os.path.join(dir_, name)
        f2 = os.path.join(dir_1, name.lower())
        shutil.move(f1, f2)
    os.removedirs(dir_)
    os.rename(dir_1, dir_)
...
if __name__ == "__main__":
    ...
    root_dir = r'D:\work\project\卡尔蔡司AR镀膜\卡尔蔡司AR模色推优数据_20210610'
    base_data_dir = os.path.join(root_dir, r'33#机台文件')
    normal_dir_name(base_data_dir)
    evt_cc_dir = os.path.join(root_dir, r'33#机台文件')
    CC_dir = os.path.join(root_dir, r'33#机台文件')
    CX_dir = os.path.join(root_dir, r'33#机台文件')
    file1 = r'D:\work\project\卡尔蔡司AR镀膜\文档s\蔡司资料0615\33#膜色文件与EVT文件对应表.xlsx'
    ...
    # .json都是数据处理中需要落盘的信息
    data_js = os.path.join(root_dir, r'0619', 'thickness_lab_curve.json')
    refine_data_json = os.path.join(root_dir, r'0619', 'refine_thickness_lab_curve.json')
    bad_js = os.path.join(root_dir, r'0619', 'bad_thick_lab.json')
    oneone_evt_thickness = os.path.join(root_dir, r'0619', 'oneone_evt_thickness.json')
    evt_33number = os.path.join(root_dir, r'0619', 'evt_33number.json')
    # 加入3维耗材信息
    num33_hc_js = os.path.join(root_dir, r'0619', '33_hc.json')
    number33_thick_js = os.path.join(root_dir, r'0619', '33number_thickness.json')
    thick_hc_lab_js = os.path.join(root_dir, r'0619', r'thick_hc_lab.json')
    # 加入16维sensor列时序特征
    thick14_hc3_sensor16_lab_js = os.path.join(root_dir, r'0619', 'thick14hc3sensor16_lab.json')
    # 加入64维 8step sensor时序特征
    csv_dict_js = os.path.join(root_dir, r'0619', 'evtname_sensor_name_value.json')
    thick14_hc3_sensor64_lab_js = os.path.join(root_dir, r'0619', 'thick14hc3sensor64_lab.json')
    # 再加入19列有意义数据的38维特征
    feature135_lab_js = os.path.join(root_dir, r'0619', 'feature135_lab.json')
