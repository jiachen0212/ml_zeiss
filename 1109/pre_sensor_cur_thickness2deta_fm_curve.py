...
def cross_val(X_train, Y_train, x_test, y_test, ind, important_inds):
...
def fix_pre_sensor_modify_thickness(x_test, important_inds, save_dir, nums=20):
    ...
    for xgb_model in xgb_models:
        ...
        preds.append(pred)
        preds_.append(pred_)
    preds = [[preds[k][j] for k in range(ll)] for j in range(nums)]
    preds_ = [[preds_[k][j] for k in range(ll)] for j in range(nums)]
    tmp = [0, 0, 0, 0, 1, 1, 0]
    ...
    for i in range(nums):
        ...
        A = [function1[k] - preds[i][k] for k in range(ll)]
        B = [function2[k] - preds_[i][k] for k in range(ll)]
...
def hope_ycy(x_test, y_test, important_inds, sensor_thick_2_OvenNo, OvenNoPair_Labs, save_dir):
    ...
    mean_ = [float(a) for a in ff[0].split(',')[:-1]]
    std_ = [float(b) for b in ff[1].split(',')[:-1]]
    x_test_no_normalize = [[test[r]*std_[r]+mean_[r] for r in range(len(std_))] for test in x_test]
    ...
    deta_y_truth = [[y_test[k][j] for k in range(ll)] for j in range(len(x_test))]
    ...
    # 真实机台的测试集曲线
    y_truths = [[functions[k][j]-deta_y_truth[k][j] for j in range(ll)] for k in range(len(x_test))]
    y_truths = y_truths[1:]
    ...
    for xgb_model in xgb_models:
        ...
        y_preds.append(pred)
    deta_y_preds = [[y_preds[k][j] for k in range(ll)] for j in range(lls-1)]
    functions_ = functions[1:]
    y_model_pred = [[functions_[k][j]-deta_y_preds[k][j] for j in range(ll)] for k in range(lls-1)]
...
def eval_test_data(x_test, important_inds, y_tests, title='train data'):
    ...
    res = [[functions[j][k] - preds[j][k] for k in range(ll)] for j in range(len(x_test))]
    gt = [[functions[j][k] - gt[j][k] for k in range(ll)] for j in range(len(x_test))]
    aa = [important_ind * 5 + 380 for important_ind in important_inds]
...
if __name__ == '__main__':
    ...
    for ind, important_ind in enumerate(important_inds):
        ...
        xgb_model = cross_val(X_train, y_train, X_test, y_test, ind, important_inds)
    ...
    save_dir = r'D:\work\project\卡尔蔡司AR镀膜\ML_ZEISS-dev\data\model_and_real_7_test'
